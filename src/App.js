...
const App = () => {
	...
	const isAppFetching = useSelector(state =>
		selectors.app.getIsFetching(state)
	);
	const isUsersFetching = useSelector(state =>
		selectors.users.getIsFetching(state)
	);
	const isPlayersFetching = useSelector(state =>
		selectors.players.getIsFetching(state)
	);
	const isMatchesFetching = useSelector(state =>
		selectors.matches.getIsFetching(state)
	);
	...
	return (
		<Router>
			<AppContainer>
				...
				<Route exact path="/matches" component={ListMatch} />
				<Route
					exact
					path="/matches/:id"
